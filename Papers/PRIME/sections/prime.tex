...
(2) Unlike common PRMs that produce only step-level rewards, the Implicit PRM provides more fine-grained \textit{token-level} rewards at no additional cost. This addresses the ambiguity in identifying steps in LLM responses while not introducing extra overhead, making it easy to combine with any RL algorithms for advantage estimation. More discussions on Implicit PRMs are in \S~\ref{sec:app_iprm}.
...
\begin{equation}
    A^i = r_o(\mathbf{y}^i)-\frac{1}{K-1} \sum_{j \neq i}r_o(\mathbf{y}^j)
\end{equation}
where $r_o(\mathbf{y}^i)$ denotes the reward of $i$-th response, $K$ is the number of samples for one prompt. The leave-one-out (LOO) baseline helps reduce variances.
...
\textbf{Updating policy with PPO loss.}
...
In practice, we find that the starting policy model itself serves as a decent initialization of PRM, bypassing the PRM training stage. This solves C3 in \S \ref{sec:challenges} and outperforms a dedicatedly trained PRM, as shown in \S~\ref{sec:design}.
